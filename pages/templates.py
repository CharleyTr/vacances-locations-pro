...
try:
    ...
    def get_templates(**kw): return []
    def save_template(d): return False
    def delete_template(i): return False
...
try:
    ...
    VARIABLES = {
        ...
        "{pays}":               "Pays du client",
        "{drapeau}":            "Drapeau du pays du client",
        ...
        "{lien_questionnaire}": "Lien questionnaire satisfaction",
...
try:
    ...
    def get_proprietes_dict(): return {}
...
def _show_templates_canal(canal: str):
    ...
    titre_form = "✏️ Modifier le modèle" if editing else f"➕ Nouveau modèle {label}"
    with st.expander(titre_form, expanded=editing is not None or not templates):
        ...
        if canal == "sms":
            ...
            st.markdown(f"<small style='color:{color}'>{nb_chars} caractères — {nb_sms} SMS</small>",
                        unsafe_allow_html=True)
        ...
        with col1:
            if st.button("💾 Enregistrer", type="primary", key=f"tpl_save_{canal}"):
                if not nom or not contenu:
                    ...
                    data = {"nom": nom, "canal": canal, "moment": moment,
                            "contenu": contenu, "actif": True}
                    if editing:
                        data["id"] = editing["id"]
