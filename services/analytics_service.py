...
def compute_monthly(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Grouper par mois d'arrivée pour le CA et le nb réservations
    # (le CA est rattaché au mois de départ pour simplifier la compta)
    ca_cols = {}
    if "prix_brut" in df_ca.columns:
        ca_cols["ca_brut"] = ("prix_brut", "sum")
    ca_cols["ca_net"] = ("prix_net", "sum")
    ca_cols["nb_reservations"] = ("id", "count")
    ca_monthly = df_ca.groupby("mois").agg(**ca_cols).reset_index()
